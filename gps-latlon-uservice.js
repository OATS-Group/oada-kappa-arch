...
function gps_uservice() {
	...
	// create topic
	producer.createTopics([gps_topic], false, function(err, data) {});
	...
	*/
	producer.on('ready', function() {
		consumer.on('message', function(message) {
			...
			// console.log(rx_buf.timestamp, rx_buf.pgn, rx_buf.data);
			...
			if (p && rx_buf.pgn === 129029) {
				...
				if (fastpacket_payloads) {
					...
					console.log(colors.red('gps_uservice | ts:', rx_buf.timestamp,
							'lat:', (data.latlb + data.lathb) * 1e-16,
							'lon:', (data.lonlb + data.lonhb) * 1e-16));
